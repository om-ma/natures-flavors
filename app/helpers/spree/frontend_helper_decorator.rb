      ...
      taxons = root_taxon.children.where(hide_from_nav: false).map do |taxon|
        # if taxon.products.present?
          ...
          content_tag :li do
            ...
            link_to(taxon.name, seo_url(taxon), data: { turbolinks: false }, 'aria-label': 'Category '+taxon.name, class: selected_taxon_klass) + (taxon.children.any? ? '<button type="button" class="dropdown-toggle js_drop_down" data-toggle="dropdown">Toggle me</button>'.html_safe : '') + taxons_tree(taxon, current_taxon, max_level - 1)
          end
        # end
      end
      safe_join(taxons, "\n")
