Spree.ready(function ($) {
  if ($('.select_address').length > 0) {
    ...
    if ($('input#order_use_shipping').is(':checked')) {
      hide_address_form('billing');
    } else {
    ...
    $('input#order_use_shipping').click(function () {
      if ($(this).is(':checked')) {
        $('#billing .select_address').hide()
        hide_address_form('billing')
  			$("#order_use_shipping_checked").val("true");
      } else {
        $('#billing .select_address').show()
        if ($("input[name='order[bill_address_id]']:checked").val() == '0') {
          show_address_form('billing')
        } else {
          hide_address_form('billing')
        }
    		$("#order_use_shipping_checked").val("false");
      }
    });
  ...
  } else if ($('#billing .inner').length > 0) {
    if ($('input#order_use_shipping').is(':checked')) {
      hide_address_form('billing');
    } else {
      show_address_form('billing');
