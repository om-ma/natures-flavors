module Spree
  class ErrorMailer < Spree::BaseMailer
    ...
    def quote_error_email(order_number, message)
      @message = message
      to_address = Rails.configuration.x.systemerror.email
      subject = "[ERROR - #{Rails.env}] Failed to get quote for Order #: #{order_number} for Route Insurance"
    ...
    def shipment_error_email(shipment_number, order_number, message)
      @message = message
      to_address = Rails.configuration.x.systemerror.email
      subject = "[ERROR - #{Rails.env}] Failed to create Shipment #: #{shipment_number}/Order #: #{order_number} for Route Insurance"
      default_options = Rails.configuration.action_mailer.default_options
